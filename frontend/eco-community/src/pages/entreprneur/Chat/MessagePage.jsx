...
import {SidebarLink} from "./components/SideBarLinks.jsx";
import {EntrepreneurConversationItem} from "./components/ConversationItem.jsx";
import {ChatMessage} from "./components/ChatMessage.jsx";
import NotificationDropdown from "./components/Notification.jsx";

const EntrepreneurChatPage = () => {
    ...
    // Helper function to safely get investor name
    const getInvestorName = (chat) => {
        if (!chat?.investor) {
            return 'Unknown Investor';
        }

        // Try to get the investor's name
        if (chat.investor.first_name && chat.investor.last_name) {
            return `${chat.investor.first_name} ${chat.investor.last_name}`;
        }

        // Try to get from user object if it exists
        if (chat.investor.user?.first_name && chat.investor.user?.last_name) {
            return `${chat.investor.user.first_name} ${chat.investor.user.last_name}`;
        }
    ...
    const getFilteredConversations = () => {
        return conversations.filter(conversation => {
            try {
                ...
                // Safely get investor name
                let investorName = '';
                if (conversation?.investor) {
                    ...
                    if (!investorName.trim() && investor.user) {
                        investorName = `${investor.user.first_name || ''} ${investor.user.last_name || ''}`;
                    }
                }
                ...
                } else if (conversation?.help_request?.project?.project_name) {
                    projectName = conversation.help_request.project.project_name;
                }
                ...
                return (
                    investorName.toLowerCase().includes(searchLower) ||
...
export default EntrepreneurChatPage